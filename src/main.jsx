...
function filterListings(homes,b,s,city){
 ...
 if(b.type==="woonlasten")return visible.filter(h=>h.status==="kansrijk").length?visible.filter(h=>h.status==="kansrijk"):visible;
...
function routeDisplay(buyer){
 ...
 if(buyer?.type==="woonlasten")return {title:"Woonlasten onder druk",text:"Eerst rustig overzicht, dan pas keuzes",kind:"sensitive"};
...
function LifeNotice({buyer}){
 ...
 if(buyer.type==="woonlasten"){
  return <div className="lifeNotice sensitive"><HeartHandshake/><div><b>We beginnen met overzicht, niet met druk.</b><p>Als woonlasten spannend worden, helpt ATLAS eerst rustig in kaart brengen wat er speelt: maandlast, buffer, woningwaarde en mogelijke vervolgstappen.</p><span>Je hoeft nu nog geen beslissing te nemen.</span></div></div>
 }
 return null;
}
...
function AtlasAssistant({buyer,context,onClose,onContinue}){
 ...
 const answers={
  ...
  "Mijn situatie is gevoelig":"Dan doen we het rustiger. ATLAS vraagt niet meer dan nodig is en past de route aan, bijvoorbeeld bij scheiding, overlijden of woonlasten onder druk.",
...
function Market({buyer,s,homes,setPage,setHomeId,setMarketFilter,next,goNext}){
 ...
 function openPlace(place){
  ...
  const first=inPlace.find(h=>h.status==="kansrijk")||inPlace.find(h=>h.status==="aandacht")||inPlace[0];
  if(first)setHomeId(first.id);
  setPage("matches");
 }
 return <section><Header kicker="ATLAS Marktinzicht" title="Waar maak je echt kans?" text="Persoonlijke marktlaag bovenop woningzoeken: aanbod, haalbaarheid en alternatieven."/><div className="marketHero"><div><h3>In jouw zoekgebied</h3><div className="marketNums"><Metric label="Actieve woningen" value="308"/><Metric label="Haalbaar binnen bandbreedte" value="134"/><Metric label="Sterke matches" value="43"/><Metric label="Topkansen" value="14"/></div></div><div className="insightCard"><Sparkles/><b>ATLAS advies</b><p>{buyer.type==="woonlasten"?"We tonen eerst alleen woningen die passen bij inkomen, vermogen en maandlastcomfort. Buiten bereik staat apart en niet als normale match.":"Baarn en Hilversum geven nu de beste combinatie van koopkans en maandlast."}</p><button onClick={()=>setPage("matches")}>Bekijk matches</button></div></div><div className="marketTable">{rows.map(r=><div className="marketRow" key={r.place}><b>{r.place}</b><span>{r.homes} woningen</span><span>{r.realistic} haalbaar</span><span>{r.top} topkansen</span><Badge value={r.pressure==="Gemiddeld"?82:r.pressure==="Hoog"?67:45}>{r.pressure}</Badge><p>{r.tip}</p><button className="rowAction" onClick={()=>openPlace(r.place)}>Bekijk woningen</button></div>)}</div><NextStep title={next.title} text={next.text} primary={next.primary} onPrimary={goNext}/></section>
}
