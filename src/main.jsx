...
function getGlobalNext(page,wizard,s,buyer){
 ...
 if(page==="share")return {title:"Paspoort klaar voor makelaar",text:"De makelaar ziet vertrouwen, score en status. Ruwe documenten blijven binnen ATLAS.",primary:"Deel met makelaar",page:"share"};
...
function Share({buyer,s,selected,next,goNext}){return <section><Header kicker="Deel mijn ATLAS" title="Live paspoort voor de makelaar." text="Jij levert documenten veilig aan ATLAS. De makelaar ontvangt vertrouwen, geen ruwe dossiers."/><div className="passportExplain"><div><b>ATLAS controleert</b><p>Loonstroken, bankafschriften, werkgeversverklaring en andere documenten worden binnen ATLAS gebruikt om je kooppositie te controleren.</p></div><div><b>De makelaar ziet</b><p>Geen BSN, loonstroken of bankafschriften. Alleen status, vertrouwen, koopkans en biedklaarheid.</p></div></div><div className="shareGrid"><div className="livePassport"><LockKeyhole/><h2>ATLAS Passport</h2><p>{buyer.firstName} {buyer.lastName}</p><strong>{s.confidence}</strong><Badge value={s.confidence}>Confidence</Badge></div><div className="shareFacts"><Fact icon={<ShieldCheck/>} label="Identiteit" value="Geverifieerd"/><Fact icon={<FileText/>} label="Documenten" value={`${s.docs}% compleet`}/>{buyer.type!=="starter"&&<Fact icon={<HousePlus/>} label="Overwaarde" value={euro(s.equity)}/>}<Fact icon={<ShieldAlert/>} label="Adviesprofiel" value={`${s.advice}% compleet`}/><Fact icon={<Home/>} label="Woning" value={selected.title}/><Fact icon={<Send/>} label="Advies" value="Plan bezichtiging"/></div></div><NextStep title={next.title} text={next.text} primary={next.primary} onPrimary={goNext}/></section>}
