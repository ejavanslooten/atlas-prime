...
function routeDisplay(buyer){
 ...
 if(buyer?.type==="woonlasten")return {title:"Financiële ruimte verkennen",text:"Jouw situatie, rustig en zonder oordeel",kind:"sensitive"};
...
function AtlasAssistant({buyer,context,onClose,onContinue}){
 ...
 const answers={
  ...
  "Mijn situatie is gevoelig":"Dan behandelen we dit extra zorgvuldig. ATLAS stelt geen onnodige vragen en gebruikt rustige taal. Je situatie is niet het probleem; het is het vertrekpunt.",
 ...
 const [messages,setMessages]=useState([{from:"atlas",text:sensitive?"Ik ben er. We doen dit rustig en discreet. Geen oordeel, geen haast — wel een duidelijke volgende stap.":"Welkom. Ik loop met je mee tijdens deze route. We houden het persoonlijk, rustig en eerlijk over wat haalbaar is."}]);
 const add=(q)=>setMessages(m=>[...m,{from:"user",text:q},{from:"atlas",text:answers[q]}]);
 return <div className="modalLayer"><div className="assistantModal concierge"><div className="assistantHead"><div><span>ATLAS Assistent</span><h2>Je persoonlijke begeleider</h2><p>Context: {context}</p></div><button className="closeMini" onClick={onClose}>×</button></div><div className="assistantBody">{messages.map((m,i)=><div key={i} className={"bubble "+m.from}>{m.text}</div>)}</div><div className="assistChoices">{Object.keys(answers).map(q=><button key={q} onClick={()=>add(q)}>{q}</button>)}</div><div className="assistFoot"><button onClick={onContinue}>Dank je, ik ga verder<ChevronRight size={16}/></button><button className="secondary" onClick={onClose}>Sluit begeleider</button></div></div></div>
}
