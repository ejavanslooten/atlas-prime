...
function routeDisplay(buyer){
 ...
 if(buyer?.type==="woonlasten")return {title:"Financiële ruimte verkennen",text:"Discreet overzicht, zonder oordeel",kind:"sensitive"};
...
function getGlobalNext(page,wizard,s,buyer){
 ...
 if(page==="profile"&&wizard===5){
  ...
  if(buyer.type!=="starter"&&s.homeReliability<75)return {title:"Controleer je huidige woning",text:"Je hebt een nieuwe-woonstap route. Bevestig eerst je woningwaarde en overwaarde.",primary:"Check mijn woning",page:"home",secondary:"Bekijk Marktinzicht",secondaryPage:"market"};
...
function App(){
 ...
 const selected=visibleHomes.find(h=>h.id===homeId)||visibleHomes[0]||allHomes[0];
 ...
 return <div className="app"><aside><div className="brand"><div className="mark">A</div><div><b>ATLAS</b><span>NEXT 3.2</span></div></div><button className="newBuyer"><Plus size={18}/> Nieuwe koper</button><div className="conciergeStart"><div className="leafGhost">⌁</div><h3>Hallo {buyer.firstName || "Erik"},<br/>fijn dat je er bent.</h3><p>Waar kan ik je vandaag mee helpen? Kies wat het beste past — ATLAS begeleidt je stap voor stap.</p></div><div className="routeCards"><button onClick={()=>start("doorstromer")} className={buyer.type==="doorstromer"?"routeCard active":"routeCard"}><HousePlus size={19}/><span><b>Nieuwe woonstap</b><small>Verken je volgende stap</small></span><ChevronRight size={17}/></button><button onClick={()=>start("starter")} className={buyer.type==="starter"?"routeCard active":"routeCard"}><UserRound size={19}/><span><b>Starter</b><small>Eerste woning kopen</small></span><ChevronRight size={17}/></button><button onClick={()=>start("zzp")} className={buyer.type==="zzp"?"routeCard active":"routeCard"}><BriefcaseBusiness size={19}/><span><b>Ondernemer / ZZP</b><small>Speciaal voor zelfstandig ondernemers</small></span><ChevronRight size={17}/></button><button onClick={()=>start("woonlasten")} className={buyer.type==="woonlasten"?"routeCard active sensitive":"routeCard sensitive"}><PieChart size={19}/><span><b>Financiële ruimte verkennen</b><small>Discreet overzicht, meer rust</small></span><ChevronRight size={17}/></button></div><nav>{navItems.map(([id,Icon,label])=><button key={id} onClick={()=>setPage(id)} className={page===id?"active":""}><Icon size={18}/>{label}</button>)}</nav><div className="progressList">{journey.map(([label,val,target],i)=><button key={label} onClick={()=>setPage(target)} className={val>=70?"ok":""}>{val>=70?<CheckCircle2 size={16}/>:<span>{i+1}</span>}<b>{label}</b><small>{val}%</small></button>)}</div></aside><main><JourneyFlow page={page} wizard={wizard} setPage={setPage} setWizard={setWizard} s={s} buyer={buyer}/>{page==="cockpit"&&<Cockpit s={s} setPage={setPage} selected={selected} buyer={buyer}/>} {page==="profile"&&<Profile buyer={buyer} update={update} s={s} wizard={wizard} setWizard={setWizard} next={next} goNext={goNext} goSecondary={goSecondary}/>} {page==="home"&&<MyHome buyer={buyer} updateBuyer={updateBuyer} s={s} docs={docsForBuyer} setDocs={setDocs} next={next} goNext={goNext}/>} {page==="documents"&&<Documents docs={docsForBuyer} setDocs={setDocs} s={s} buyer={buyer} next={next} goNext={goNext}/>} {page==="advice"&&<Advice buyer={buyer} update={update} s={s} next={next} goNext={goNext}/>} {page==="market"&&<Market buyer={buyer} s={s} homes={allHomes} setPage={setPage} setHomeId={setHomeId} setMarketFilter={setMarketFilter} next={next} goNext={goNext}/>} {page==="matches"&&<Matches homes={visibleHomes} allHomes={allHomes} marketFilter={marketFilter} setMarketFilter={setMarketFilter} buyer={buyer} homeId={homeId} setHomeId={setHomeId} selected={selected} s={s} next={next} goNext={goNext}/>} {page==="coach"&&<Coach buyer={buyer} s={s} selected={selected} setPage={setPage} next={next} goNext={goNext}/>} {page==="share"&&<Share buyer={buyer} s={s} selected={selected} next={next} goNext={goNext}/>} {page==="agent"&&<AgentView buyer={buyer} s={s} selected={selected} next={next} goNext={goNext}/>}</main><button className="assistantFloat" onClick={()=>setAssistOpen(true)}><Bot size={20}/><span><b>ATLAS Assistent</b><small>Ik kijk met je mee</small></span></button>{checkInOpen&&<CheckIn buyer={buyer} onContinue={()=>{setCheckInOpen(false);proceedTo(next)}} onAdjust={()=>{setCheckInOpen(false);setPage("profile");setWizard(Math.max(0,wizard-1))}} onAssist={()=>{setAssistOpen(true)}} onClose={()=>setCheckInOpen(false)}/>} {assistOpen&&<AtlasAssistant buyer={buyer} context={stepTitle(page,wizard,buyer)} onClose={()=>setAssistOpen(false)} onContinue={()=>{setAssistOpen(false);setCheckInOpen(false);proceedTo(next)}}/>}</div>
}
...
function Cockpit({s,setPage,selected,buyer}){
 ...
 const buttonText=query.length>=2?`Check koopkans in ${query}`:"Start kooproute";
 return <section><div className="atlasHero"><div><span className="kicker">ATLAS Confidence Engine</span><h1>Duidelijkheid over je kooppositie.</h1><p className="lead">ATLAS helpt je begrijpen wat haalbaar is, welke stappen ontbreken en hoe voorbereid je bent voordat je serieus gaat bieden.</p><div className="heroSearchWrap"><div className="heroSearch"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Zoek plaats, buurt, postcode of woning"/><button onClick={()=>setPage(query.length>=2?"market":"profile")}>{buttonText}</button></div>{suggestions.length>0&&<div className="homeSuggest">{suggestions.map(x=><button key={x.type+x.label} onClick={()=>selectSuggestion(x)}><MapPin size={16}/><div><b>{x.label}</b><span>{x.type} · {x.sub}</span></div></button>)}</div>}</div><div className="heroModes"><span>Kopen</span><span>Nieuwe woonstap</span><span>Starter</span><span>Financiële ruimte verkennen</span></div></div><div className="scoreOrb"><span>{buyer.firstName} is koopklaar</span><strong>{s.atlas}</strong><Bar value={s.atlas}/><small>Confidence: {s.confidence}%</small></div></div><div className="today"><Action title="Dynamische Document Vault" points={`${s.docs}% compleet`} text="Elke route krijgt automatisch de juiste documenten." icon={<FileText/>}/><Action title="Marktinzicht" points="Nieuw" text="Zie waar je in jouw regio de meeste koopkans hebt." icon={<BarChart3/>}/><Action title="Beste match vandaag" points={`${selected.match}% match`} text={`${selected.title} in ${selected.city}`} icon={<Home/>}/></div></section>
}
...
function CheckIn({buyer,onContinue,onAdjust,onAssist,onClose}){
 ...
 return <div className="modalLayer"><div className="checkCard"><button className="closeMini" onClick={onClose}>×</button><span className="kicker">{sensitive?"Even rustig checken":"Even checken"}</span><h2>{sensitive?"We doen dit stap voor stap.":"Je bent goed op weg."}</h2><p>{sensitive?"Sommige vragen kunnen persoonlijk zijn. ATLAS helpt je verder zonder oordeel en zonder haast, maar wel met een duidelijke route naar overzicht.":"We hebben nu genoeg om je kooppositie scherper te maken. Je hoeft het niet perfect te weten; vul in wat je nu het beste weet."}</p>{why&&<div className="whyBox"><Info/><div><b>Waarom vraagt ATLAS dit?</b><p>Deze informatie helpt om je kooppositie betrouwbaarder te maken. Onzekere antwoorden markeren we als ‘nog te controleren’ en scherpen we later aan met documenten.</p></div></div>}<div className="checkActions"><button onClick={onContinue}>{sensitive?"Ga rustig verder":"Ga door naar de volgende stap"}<ChevronRight size={16}/></button><button className="secondary" onClick={onAssist}>Ik wil verder met extra ondersteuning</button><button className="soft" onClick={()=>setWhy(!why)}>Leg uit waarom dit nodig is</button><button className="soft" onClick={onAdjust}>Ik wil iets aanpassen</button></div></div></div>
}

function AtlasAssistant({buyer,context,onClose,onContinue}){
 const sensitive=buyer.maritalStatus==="Weduwe/weduwnaar"||buyer.maritalStatus==="Gescheiden"||buyer.type==="woonlasten";
 const answers={
  "Ik weet niet wat ik moet invullen":"Vul in wat je nu het beste weet. ATLAS gebruikt dit als eerste inschatting en markeert onderdelen die later met documenten gecontroleerd moeten worden.",
  "Waarom is dit nodig?":"Omdat een makelaar straks niet jouw privé-documenten ziet, maar wel moet kunnen vertrouwen op een gecontroleerde kooppositie.",
  "Mijn situatie is gevoelig":"Dan doen we het rustiger. ATLAS vraagt niet meer dan nodig is en past de route aan, bijvoorbeeld bij scheiding, overlijden of financiële ruimte verkennen.",
  "Wat ziet de makelaar?":"De makelaar ziet status, kooppositie, biedklaarheid en aandachtspunten. Geen BSN, loonstroken, bankafschriften of ruwe documenten."
 };
 const [messages,setMessages]=useState([{from:"atlas",text:sensitive?"Natuurlijk. We doen dit rustig. Ik help je verder zonder oordeel, met aandacht voor jouw situatie en steeds één duidelijke volgende stap.":"Welkom. Ik blijf tijdens elke stap bij je. We houden het rustig, duidelijk en zorgvuldig, maar ik help je wel vooruit."}]);
 const add=(q)=>setMessages(m=>[...m,{from:"user",text:q},{from:"atlas",text:answers[q]}]);
 return <div className="modalLayer"><div className="assistantModal"><div className="assistantHead"><div><span>ATLAS Assistent</span><h2>Je persoonlijke ATLAS begeleider</h2><p>Context: {context}</p></div><button className="closeMini" onClick={onClose}>×</button></div><div className="assistantBody">{messages.map((m,i)=><div key={i} className={"bubble "+m.from}>{m.text}</div>)}</div><div className="assistChoices">{Object.keys(answers).map(q=><button key={q} onClick={()=>add(q)}>{q}</button>)}</div><div className="assistFoot"><button onClick={onContinue}>Ik snap het, ga verder<ChevronRight size={16}/></button><button className="secondary" onClick={onClose}>Sluit assistent</button></div></div></div>
}
