...
function getGlobalNext(page,wizard,s,buyer){
 ...
 if(page==="profile"&&wizard<5){
  return {title:"Stap compleet",text:`Je bent bij stap ${wizard+1} van ${steps.length}. De volgende stap past zich aan jouw situatie aan. Ga rustig verder naar ${steps[wizard+1].label}.`,primary:buyer.maritalStatus==="Weduwe/weduwnaar"&&wizard===0?"Ga rustig verder":`Ga naar ${steps[wizard+1].label}`,page:"profile",wizard:wizard+1};
 }
 if(page==="profile"&&wizard===5){
...
function householdStepLabel(buyer){
 ...
 if(buyer?.type==="starter" && buyer?.maritalStatus==="Alleenstaand")return "Woonsituatie";
 return "Huishouden";
}
function householdStepText(buyer){
 ...
 if(buyer?.type==="starter" && buyer?.maritalStatus==="Alleenstaand")return "We kijken alleen of er mensen of lasten zijn die invloed hebben op je woonruimte of maandlasten.";
...
function AtlasAssistant({buyer,context,onClose,onContinue}){
 ...
 const answers={
  ...
  "Mijn situatie is persoonlijk":"Dan behandelen we dit extra zorgvuldig. ATLAS stelt geen onnodige vragen en gebruikt rustige taal. Je situatie is niet het probleem; het is het vertrekpunt.",
 ...
 return <div className="modalLayer"><div className="assistantModal concierge refined"><div className="assistantHead"><div><span>ATLAS Assistent</span><h2>Je persoonlijke begeleider</h2><p>Context: {context==="Gezin"?"Huishouden":context}</p></div><button className="closeMini" onClick={onClose}>×</button></div><div ref={bodyRef} className="assistantBody refined">{messages.map((m,i)=><div key={i} className={"bubble "+m.from}>{m.text}</div>)}</div><div className="assistChoices refined">{Object.keys(answers).map(q=><button key={q} onClick={()=>add(q)}>{q}</button>)}</div><div className="assistFoot"><button onClick={onContinue}>Dank je, ik ga verder<ChevronRight size={16}/></button><button className="secondary" onClick={onClose}>Sluit begeleider</button></div></div></div>
}
