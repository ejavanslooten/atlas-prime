...
function AtlasAssistant({buyer,context,onClose,onContinue}){
 ...
 const [messages,setMessages]=useState([{from:"atlas",text:sensitive?"Natuurlijk. We doen dit rustig. Ik help je verder zonder druk, maar wel stap voor stap richting overzicht.":"Natuurlijk. Ik help je stap voor stap. We houden de route eenvoudig en zorgen dat je in beweging blijft."}]);
 const add=(q)=>setMessages(m=>[...m,{from:"user",text:q},{from:"atlas",text:answers[q]}]);
 return <div className="modalLayer"><div className="assistantModal"><div className="assistantHead"><div><span>ATLAS Assistent</span><h2>Je persoonlijke koopbegeleider</h2><p>Context: {context}</p></div><button className="closeMini" onClick={onClose}>×</button></div><div className="assistantBody">{messages.map((m,i)=><div key={i} className={"bubble "+m.from}>{m.text}</div>)}</div><div className="assistChoices">{Object.keys(answers).map(q=><button key={q} onClick={()=>add(q)}>{q}</button>)}</div><div className="assistFoot"><button onClick={onContinue}>Ik snap het, ga verder<ChevronRight size={16}/></button><button className="secondary" onClick={onClose}>Sluit assistent</button></div></div></div>
}
...
function Profile({buyer,update,s,wizard,setWizard,next,goNext,goSecondary}){
 ...
 {wizard===1&&<><h3>Gezinssituatie</h3>{sensitiveLife&&<div className="lifeNotice sensitive compact"><HeartHandshake/><div><b>We passen deze stap aan jouw situatie aan.</b><p>ATLAS vraagt hier niet naar een partner. We kijken alleen naar personen die financieel afhankelijk zijn en wat nodig is voor overzicht.</p></div></div>}<div className="toggleGrid">{!sensitiveLife&&<Toggle label="Partner aanwezig" checked={buyer.partner} onChange={v=>update("partner",v)}/>}<Toggle label={sensitiveLife?"Kinderen of financieel afhankelijk":"Kinderen"} checked={buyer.children} onChange={v=>update("children",v)}/><Toggle label="Co-ouderschap" checked={buyer.coParenting} onChange={v=>update("coParenting",v)}/><Toggle label="Alimentatie" checked={buyer.alimony} onChange={v=>update("alimony",v)}/></div>{!sensitiveLife&&buyer.partner&&<div className="two"><Field label="Naam partner" value={buyer.partnerName} onChange={v=>update("partnerName",v)}/><Field label="Geboortedatum partner" value={buyer.partnerBirthDate} onChange={v=>update("partnerBirthDate",v)}/></div>}{buyer.children&&<div className="two"><Field label="Aantal kinderen / afhankelijken" type="number" value={buyer.childCount} onChange={v=>update("childCount",v)}/><Field label="Leeftijden" value={buyer.childAges} onChange={v=>update("childAges",v)}/></div>}<Field label="Kinderopvang / zorgkosten p/m" prefix="€" type="number" value={buyer.childcare} onChange={v=>update("childcare",v)}/>{sensitiveLife&&<div className="pensionBox"><h3>Aanvullende zekerheid</h3><p>Alleen invullen als dit relevant is voor jouw situatie.</p><Field label="Nabestaandenpensioen / uitkering p/j" prefix="€" type="number" value={buyer.pensionIncome} onChange={v=>update("pensionIncome",v)}/></div>}</>}
