...
const profiles={
 ...
 private:{
  label:"ATLAS Private",
  buyer:{type:"private",firstName:"Private",lastName:"Client",birthDate:"1976-03-22",nationality:"Nederlands",maritalStatus:"Gehuwd",maritalProperty:"Huwelijkse voorwaarden",partner:true,partnerName:"Partner",partnerBirthDate:"1978-11-04",children:true,childCount:2,childAges:"13, 16",coParenting:false,alimony:false,childcare:0,workStatus:"DGA / eigen bedrijf",employer:"Holding / eigen onderneming",role:"Ondernemer",income:145000,partnerIncome:68000,holidayPay:false,thirteenthMonth:false,bonus:0,extraIncomeOpen:true,extraIncomeType:"Dividend / management fee",extraIncome:45000,probation:false,pensionAge:67,pensionIncome:0,partnerPension:0,pensionDrop:"Onbekend",cash:325000,investments:420000,crypto:0,studyDebt:0,loans:0,creditcard:0,leaseMonthly:0,bkr:"Geen negatieve registratie",address:"Rijksweg 8, Laren",city:"Laren",postcode:"1251 AB",lat:"52.2564",lng:"5.2256",homeValue:1325000,woz:1180000,mortgageLeft:520000,sellingCosts:34000,saleStatus:"Stille verkoop / nog niet publiek",mortgageMonthly:2650,rate:3.4,energyLabel:"A",homeTypeCurrent:"Vrijstaand",buildYear:2004,homeImage:"https://images.unsplash.com/photo-1600607687920-4e2a09cf159d?auto=format&fit=crop&w=1200&q=80",places:"Laren, Blaricum, Baarn, Bussum",radius:18,homeType:"Vrijstaand, Twee-onder-een-kap",rooms:6,garden:true,garage:true,energy:"A of beter",minPrice:950000,maxPrice:1650000,moveInReady:true,adviceGoal:"Discrete aankoop hogere segment",maxMonthly:5200,currentMonthly:2650,leftOverMonthly:4200,riskProfile:"Discretie en zekerheid belangrijk",ratePreference:"Maatwerk rente en zekerheid",repaymentPreference:"Maatwerk op vermogen en cashflow",deathScenario:"Continuïteit gezin en vermogen borgen",unemploymentScenario:"Niet van toepassing / ondernemersrisico",disabilityScenario:"Continuïteit inkomen en vermogen borgen",futurePlans:"Discrete aankoop in hoger segment met beperkte zichtbaarheid naar markt en makelaar.",expectedIncome:"Variabel maar sterk onderbouwbaar"}
 }
...
function docRules(buyer){
 ...
 if((buyer.type==="doorstromer"||buyer.type==="woonlasten"||buyer.type==="private") && buyer.saleStatus!=="Geen huidige woning"){
 ...
 if(buyer.type==="private"){
  docs.push(["privateAssets","Vermogensoverzicht / portefeuille","ATLAS Private",0]);
  docs.push(["privateSource","Herkomst eigen middelen","ATLAS Private",0]);
  docs.push(["privateHolding","Holding-, dividend- of management fee bewijs","ATLAS Private",0]);
  docs.push(["privateDiscretion","Discretie- en deelinstellingen bevestigd","ATLAS Private",0]);
 }
...
function short(n){
 const v=Number(n)||0;
 if(Math.abs(v)>=1000000)return "€ "+(v/1000000).toLocaleString("nl-NL",{maximumFractionDigits:1,minimumFractionDigits:1})+" mln";
 if(Math.abs(v)>=100000)return "€ "+Math.round(v/1000).toLocaleString("nl-NL")+"k";
 return euro(v);
}
...
function budget(b,docs){
 ...
 const wealthFactor=b.type==="private"?((+b.cash||0)*.92+(+b.investments||0)*.70):((+b.cash||0)*.82+(+b.investments||0)*.55);
 return Math.max(0,Math.round(income*4.35+wealthFactor+usable(b)-penalty));
}
...
function verificationStage(doc,buyer){
 const p=doc?.progress||0;
 if(p>=100)return {label:"Bronbevestigd",tone:"ok",score:4,text:"Gegevens zijn volledig onderbouwd of in pilot handmatig gevalideerd."};
 if(p>=70)return {label:"Consistent",tone:"ok",score:3,text:"Document past bij andere gegevens; laatste controle of bronkoppeling volgt."};
 if(p>=35 || doc?.status==="review")return {label:"Uitgelezen",tone:"wait",score:2,text:"Velden zijn herkend; ATLAS vergelijkt deze met profiel en andere documenten."};
 if(p>0)return {label:"Ontvangen",tone:"wait",score:1,text:"Document is aanwezig, maar nog niet inhoudelijk verwerkt."};
 return {label:"Ontbreekt",tone:"miss",score:0,text:"Nog nodig voor een sterker en eerlijker koopprofiel."};
}
function routeVerificationText(buyer){
 if(buyer?.type==="starter")return "Inkomen, eigen middelen en salarisbijschrijving worden gecontroleerd. Geen huidige-woning-documenten.";
 if(buyer?.type==="zzp")return "Jaarstukken, IB-aangiftes, prognose en bankdata moeten samen een stabiel inkomen onderbouwen.";
 if(buyer?.type==="private")return "Vermogen, herkomst eigen middelen, holding/dividend en discretie-instellingen worden extra zorgvuldig behandeld.";
 if(buyer?.type==="woonlasten")return "Maandlasten, hypotheekafschrijving, woningwaarde en betaalbaarheid krijgen extra nadruk.";
 return "Inkomen, werkgeversverklaring, salarisbijschrijving en huidige woningpositie worden tegen elkaar gecontroleerd.";
}
function routeExecutionMode(buyer){
 if(buyer?.type==="private")return "Hybride pilot: documentuitlezing + menselijke validatie + discrete deelinstellingen.";
 if(buyer?.type==="zzp")return "Hybride pilot: jaarstukken handmatig valideren, daarna automatiseren via partners.";
 return "Hybride pilot: loonstroken, werkgeversverklaring en bankbijschrijving inhoudelijk vergelijken.";
}
function criticalChecks(buyer){
 const base=[
  ["Loonstrook","Naam, werkgever, periode, bruto/netto salaris, vaste toeslagen en inhoudingen."],
  ["Werkgeversverklaring","Recente datum, contracttype, functie, inkomen, intentieverklaring en bedrijfsgegevens."],
  ["Bankbijschrijving","Netto salarisbedrag en naam werkgever moeten logisch aansluiten op loonstrook."],
  ["Profielconsistentie","Opgegeven inkomen en gezinssituatie mogen niet botsen met documenten."]
 ];
 if(buyer?.type==="starter")return base.filter(x=>x[0]!=="Profielconsistentie").concat([["Eigen middelen","Spaargeld en beschikbare middelen worden onderbouwd zonder overwaarde-route."]]);
 if(buyer?.type==="zzp")return [["Jaarstukken","Omzet, winst en stabiliteit over meerdere jaren."],["IB-aangiftes","Aansluiting met opgegeven inkomen en beschikbare buffer."],["Bankdata","Inkomstenstroom en zakelijke/privé scheiding."],["Handmatige review","Afwijkingen worden eerst door expert of partner beoordeeld."]];
 if(buyer?.type==="private")return [["Vermogen","Portefeuille, liquiditeit en inzetbaarheid."],["Herkomst middelen","Onderbouwing waar eigen middelen vandaan komen."],["Holding/dividend","Cashflow, management fee en dividend zijn niet hetzelfde als vast salaris."],["Discretie","Makelaar ziet status, niet het private dossier."]];
 return base.concat([["Huidige woning","Woningwaarde, hypotheekrestant, verkoopstatus en verkoopkosten."]]);
}
...
function routeForBuyer(buyer){
 ...
 if(buyer?.type!=="starter"){
  base.push({key:"home",label:buyer?.type==="private"?"Private woningpositie":"Mijn woning",page:"home"});
 }
 base.push(
  {key:"documents",label:buyer?.type==="private"?"Private documenten":"Documenten",page:"documents"},
  {key:"verification",label:"Verificatie",page:"verification"},
...
function routeDisplay(buyer){
 ...
 if(buyer?.type==="private")return {title:"ATLAS Private actief",text:"Discrete hogere-segment route",kind:"private"};
...
function getGlobalNext(page,wizard,s,buyer){
 ...
 if(page==="documents"){
  ...
  return {title:"Documenten gecontroleerd",text:"Laat nu zien hoe ATLAS controleert: documentuitlezing, consistentiechecks, pilotvalidatie en partnerkoppelingen.",primary:"Bekijk Verificatie",page:"verification"};
 }
 if(page==="verification")return {title:"Verificatielaag bekend",text:"Nu kun je woonwensen bekijken met duidelijkheid over wat echt gecontroleerd is en wat nog pilotmatig of partnerafhankelijk is.",primary:"Ga naar Woonwensen",page:"profile",wizard:4};
...
function routeHumanTone(buyer){
 ...
 if(buyer.type==="private")return {title:"Meer complexiteit vraagt om meer zorgvuldigheid.",text:"ATLAS Private ondersteunt hogere segmenten, vermogen, holdingstructuren en discrete koopposities zonder onnodige zichtbaarheid."};
...
function App(){
 ...
 const journeyBase=[["Persoonlijk",s.personal,"profile"],[householdStepLabel(buyer),s.family,"profile"],["Werk",buyer.income?96:35,"profile"],["Financiën",s.financial,"profile"],["Documenten",s.docs,"documents"],["Verificatie",Math.min(100,Math.max(20,s.docs+10)),"verification"],["Adviesprofiel",s.advice,"advice"],["Marktinzicht",82,"market"],["Woningen",91,"matches"],["Paspoort",68,"share"]];
 const journey=hasCurrentHome(buyer)?[["Persoonlijk",s.personal,"profile"],[householdStepLabel(buyer),s.family,"profile"],["Werk",buyer.income?96:35,"profile"],[buyer.type==="private"?"Private woningpositie":"Eigen woning",s.home,"home"],["Financiën",s.financial,"profile"],["Documenten",s.docs,"documents"],["Verificatie",Math.min(100,Math.max(20,s.docs+10)),"verification"],["Adviesprofiel",s.advice,"advice"],["Marktinzicht",82,"market"],["Woningen",91,"matches"],["Paspoort",68,"share"]]:journeyBase;
 const navItems=[["cockpit",Compass,"Cockpit"],["profile",UserRound,"Gegevens"],["home",HousePlus,buyer.type==="private"?"Private woning":"Mijn woning"],["documents",UploadCloud,buyer.type==="private"?"Private documenten":"Documenten"],["verification",ShieldCheck,"Verificatie"],["advice",ShieldAlert,"Adviesprofiel"],["market",BarChart3,"Marktinzicht"],["matches",Home,"Matches"],["coach",Bot,"Assistent"],["share",Share2,"Paspoort"],["agent",Building2,"Makelaar"]].filter(([id])=>hasCurrentHome(buyer)||id!=="home");
...
function AgentView({buyer,s,selected,next,goNext}){
 ...
 const isStarter=buyer.type==="starter";
 const isPrivate=buyer.type==="private";
 const facts=isStarter?[
  ["Identiteit","Geverifieerd"],
  ["Documenten",`${s.docs}% compleet`],
  ["Financiële basis","Inkomen en eigen middelen"],
  ["Indicatieve bandbreedte",short(s.budget)],
  ["Voorkeurswoning",selected.title]
 ]:isPrivate?[
  ["Identiteit","Geverifieerd"],
  ["Private documenten",`${s.docs}% compleet`],
  ["Vermogenspositie","Gecontroleerd / discreet"],
  ["Woningpositie",euro(s.equity)],
  ["Indicatieve bandbreedte",short(s.budget)],
  ["Voorkeurswoning",selected.title]
 ]:[
  ["Identiteit","Geverifieerd"],
  ["Documenten",`${s.docs}% compleet`],
  ["Overwaarde",euro(s.equity)],
  ["Indicatieve bandbreedte",short(s.budget)],
  ["Voorkeurswoning",selected.title]
 ];
 return <section><Header kicker="Makelaarview" title="Wat ziet de makelaar?" text={isPrivate?"Deze weergave is bewust zakelijk, privacy-arm en discreet. Alleen relevante status en onderbouwing worden gedeeld.":"Deze weergave is bewust zakelijk en privacy-arm. De makelaar krijgt vertrouwen, geen privé-dossier."}/><div className="agentGrid"><div className="agentPassport"><div className="agentHeader"><div><span>ATLAS koperprofiel</span><h2>{buyer.firstName} {buyer.lastName}</h2><p>{routeDisplay(buyer).title}</p></div><Badge value={s.confidence}>{ready?"Biedklaar":"Aandacht nodig"}</Badge></div><div className="agentScore"><strong>{s.confidence}%</strong><div><b>{isVerifiedScore(s)?"Confidence":"Voorlopige confidence"}</b><Bar value={s.confidence}/><p>{isVerifiedScore(s)?"Gebaseerd op profiel, documenten, woonwensen en kooppositie.":"Nog niet volledig onderbouwd; documenten bepalen de betrouwbaarheid."}</p></div></div><div className="agentFacts">{facts.map(([label,value])=><Fact key={label} icon={label.includes("Identiteit")?<ShieldCheck/>:label.includes("Document")?<FileText/>:label.includes("woning")||label.includes("Woning")||label.includes("Overwaarde")?<HousePlus/>:<BarChart3/>} label={label} value={value}/>)}</div></div><div className="agentSide"><h3>{isPrivate?"Waarom waardevol voor private makelaars?":"Waarom waardevol?"}</h3><p>{isPrivate?"De makelaar ziet of een hogere-segment koper serieus, discreet voorbereid en financieel onderbouwd is — zonder onnodige privé-informatie.":"De makelaar ziet sneller of een koper serieus, voorbereid en realistisch is. Dat bespaart tijd en voorkomt onnodige onzekerheid in het biedproces."}</p><div className="privacySplit vertical"><div><b>Makelaar ziet</b><span>{isPrivate?"Status, kooppositie, documentpercentages, discretieniveau en aandachtspunten.":"Status, koopkans, biedklaarheid, documentpercentages en aandachtspunten."}</span></div><div><b>Makelaar ziet niet</b><span>BSN, loonstroken, bankafschriften, ruwe documenten en privé-details.</span></div></div><button onClick={goNext}>Terug naar Cockpit<ChevronRight size={16}/></button></div></div><NextStep title={next.title} text={next.text} primary={next.primary} onPrimary={goNext}/></section>
}
...
function Documents({docs,setDocs,s,buyer,next,goNext}){
 ...
 const routeText=routeVerificationText(buyer);
 return <section><Header kicker="Document Vault + Verification Layer" title="Veilig bewijs voor je kooppositie." text="ATLAS verzamelt niet alleen documenten. Elk document doorloopt een controlelaag: ontvangen, uitgelezen, consistent en uiteindelijk bronbevestigd of handmatig gevalideerd."/><div className="vaultTop execution"><div><h3>Je documentstatus is {s.docs}% compleet.</h3><p>{buyer.type==="starter"?"Starterprofiel: geen huidige woning documenten.":buyer.type==="woonlasten"?"Financiële-ruimte route: woningwaarde, hypotheek en maandlasten extra zorgvuldig in beeld.":buyer.type==="doorstromer"?"Nieuwe-woonstap profiel: huidige woning documenten actief.":buyer.type==="private"?"ATLAS Private: vermogen, herkomst middelen en discretie krijgen een aparte controlelaag.":"ZZP-profiel: jaarstukken en IB-aangiftes actief."}</p><p>{routeText}</p><p>Nog {missing.length} documenten tot een sterker ATLAS Paspoort.</p><Bar value={s.docs}/></div><div className="verifyMini"><span>Harde regel</span><b>Upload ≠ goedgekeurd</b><p>Een document verhoogt vertrouwen pas als het inhoudelijk past bij andere gegevens.</p></div></div><div className="verificationSteps"><div><span>1</span><b>Ontvangen</b><small>Bestand veilig binnen</small></div><div><span>2</span><b>Uitgelezen</b><small>Velden herkend</small></div><div><span>3</span><b>Consistent</b><small>Gecheckt tegen andere data</small></div><div><span>4</span><b>Bronbevestigd</b><small>Partnerdata of expertvalidatie</small></div></div><div className="docGrid">{docs.map(d=>{const stage=verificationStage(d,buyer);return <div className="doc verifiedDoc" key={d.id}><div className={"statusIcon "+(d.status==="verified"?"ok":d.status==="review"?"wait":"miss")}>{d.status==="verified"?<CheckCircle2/>:d.status==="review"?<Clock3/>:<AlertTriangle/>}</div><div><b>{d.name}</b><small>{d.type} · {d.required?`${d.uploaded||0} van ${d.required} geüpload`:d.status==="verified"?"Geverifieerd":d.status==="review"?"AI controle bezig":"Ontbreekt"}</small><Bar value={d.progress}/><div className={"verificationTag "+stage.tone}><span>{stage.label}</span><em>{stage.text}</em></div></div><Badge value={d.progress}>{d.progress}%</Badge>{d.status==="verified"?<button className="soft"><Eye size={16}/> Bekijk status</button>:<button onClick={()=>upload(d.id)}><UploadCloud size={16}/> Upload</button>}{d.status==="missing"&&<button className="soft" onClick={()=>review(d.id)}>AI check</button>}<button className="iconbtn light" onClick={()=>remove(d.id)}><Trash2 size={17}/></button></div>})}</div><NextStep title={next.title} text={next.text} primary={next.primary} onPrimary={goNext}/></section>
}

function VerificationBlueprint({docs,s,buyer,next,goNext}){
 const checks=criticalChecks(buyer);
 const phases=[
  ["Fase 1","Werkende demo","UX, routes, statussen en gesimuleerde controles laten de waarde ervaren.","Nu"],
  ["Fase 2","Hybride pilot","Echte uploads, automatische uitlezing waar mogelijk en menselijke/partnercontrole bij twijfel.","Volgende stap"],
  ["Fase 3","Partnerkoppelingen","Bankdata, salarisbijschrijving, brondata en erkende validatiepartners aansluiten.","Na pilot"],
  ["Fase 4","Schaalbaar platform","Automatische documentcontrole, risicosignalen, makelaar-dashboard en ATLAS Paspoort op schaal.","Schaal"]
 ];
 const makelaar=buyer.type==="private"?"De makelaar ziet status, discretieniveau, kooppositie en aandachtspunten — geen private documenten.":buyer.type==="starter"?"De makelaar ziet financiële basis, documentstatus, kooppositie en aandachtspunten — geen loonstroken of bankafschriften.":"De makelaar ziet gecontroleerde status, kooppositie, documentpercentages en aandachtspunten — geen ruwe documenten.";
 return <section><Header kicker="ATLAS Execution & Verification Blueprint" title="Van demo naar uitvoerbare pilot." text="Deze laag maakt ATLAS verdedigbaar tijdens serieuze gesprekken. We laten zien wat nu demo is, wat pilotmatig kan en wat later via partners schaalbaar wordt."/><div className="blueprintHero"><div><Sparkles/><h3>{routeDisplay(buyer).title}</h3><p>{routeVerificationText(buyer)}</p><strong>{routeExecutionMode(buyer)}</strong></div><div><LockKeyhole/><h3>Claimgrens</h3><p>ATLAS geeft geen hypotheekadvies en vervangt geen bankcontrole. ATLAS bouwt eerder bewijs op en maakt zichtbaar hoe goed een kooppositie onderbouwd is.</p></div></div><div className="phaseGrid">{phases.map(([nr,title,text,tag])=><div className="phaseCard" key={nr}><span>{nr}</span><h3>{title}</h3><p>{text}</p><b>{tag}</b></div>)}</div><div className="executionGrid"><div className="panel"><h3>Controleerbare checks</h3><div className="checkList">{checks.map(([title,text])=><div key={title}><CheckCircle2/><span><b>{title}</b><small>{text}</small></span></div>)}</div></div><div className="panel"><h3>Wat ziet de makelaar?</h3><p>{makelaar}</p><div className="privacySplit vertical"><div><b>Wel zichtbaar</b><span>Status, documentcontrole, kooppositie, confidence en aandachtspunten.</span></div><div><b>Niet zichtbaar</b><span>BSN, loonstroken, bankafschriften, ruwe documenten en onnodige inkomensdetails.</span></div></div></div></div><div className="executionRules"><h3>Uitvoerbaarheidsregels voor ATLAS 3.8</h3><div><b>1. Geen schijnzekerheid</b><span>Voor documenten voldoende zijn, blijft alles voorlopig.</span></div><div><b>2. Upload is geen goedkeuring</b><span>Documenten moeten inhoudelijk kloppen met andere bronnen.</span></div><div><b>3. Pilot mag hybride zijn</b><span>Handmatige validatie is toegestaan zolang ATLAS eerlijk toont wat wel/niet geautomatiseerd is.</span></div><div><b>4. Partnerdata komt later</b><span>Bronbevestiging wordt schaalbaar via bankdata, salarisdata en validatiepartners.</span></div></div><NextStep title={next.title} text={next.text} primary={next.primary} onPrimary={goNext}/></section>
}
...
function Share({buyer,s,selected,next,goNext}){return <section><Header kicker="ATLAS Paspoort" title="Deel vertrouwen, niet je privé-dossier." text={isVerifiedScore(s)?"Privacy is geen bijzaak. ATLAS maakt jouw kooppositie deelbaar zonder dat ruwe documenten standaard bij een makelaar terechtkomen.":"Je paspoort blijft voorlopig totdat documenten voldoende zijn gecontroleerd. ATLAS deelt geen schijnzekerheid."}/><div className="trustExperience"><div><LockKeyhole/><h3>Jij houdt regie</h3><p>ATLAS laat vooraf zien wat gedeeld wordt, waarom dit relevant is en wat privé blijft.</p></div><div><ShieldCheck/><h3>De makelaar krijgt zekerheid</h3><p>Status, documentcontrole, kooppositie en biedklaarheid — zonder loonstroken of bankafschriften.</p></div><div><HeartHandshake/><h3>{buyer.type==="private"?"Discreet":"Oordeelvrij"}</h3><p>{buyer.type==="private"?"ATLAS Private toont alleen wat nodig is voor vertrouwen, met strengere deelinstellingen.":"Ook bij complexe situaties blijft de taal rustig, discreet en menselijk."}</p></div></div><div className="passportHero"><div className="passportCardPro"><div className="passportTop"><LockKeyhole/><span>Gedeeld profiel</span></div><h2>{buyer.firstName} {buyer.lastName}</h2><p>{routeDisplay(buyer).title} · laatst bijgewerkt vandaag</p><strong>{s.confidence}%</strong><Bar value={s.confidence}/><small>ATLAS Confidence</small></div><div className="passportNarrative"><h3>Wat betekent dit voor de makelaar?</h3><p>De koper is stap voor stap voorbereid. ATLAS toont welke onderdelen zijn aangeleverd, gecontroleerd of nog aandacht vragen.</p><div className="privacySplit"><div><b>Wel zichtbaar</b><span>Kooppositie, biedklaarheid, documentstatus, Confidence en aandachtspunten.</span></div><div><b>Niet zichtbaar</b><span>BSN, loonstroken, bankafschriften, ruwe documenten en onnodige inkomensdetails.</span></div></div></div></div><div className="shareGrid refined"><div className="shareFacts"><Fact icon={<ShieldCheck/>} label="Identiteit" value="Geverifieerd"/><Fact icon={<FileText/>} label="Documentstatus" value={`${s.docs}% compleet`}/>{hasCurrentHome(buyer)&&<Fact icon={<HousePlus/>} label={buyer.type==="private"?"Private woningpositie":"Overwaarde"} value={buyer.type==="private"?euro(s.equity)+" discreet":euro(s.equity)}/>}<Fact icon={<ShieldAlert/>} label="Adviesprofiel" value={`${s.advice}% compleet`}/><Fact icon={<Home/>} label="Woningmatch" value={selected.title}/><Fact icon={<Send/>} label="Status" value={s.docs>=80?"Biedklaar":"Bijna biedklaar"}/></div><div className="humanCopy"><HeartHandshake/><h3>Menselijke uitleg</h3><p>ATLAS gebruikt financiële taal, maar blijft begrijpelijk. Bij gevoelige situaties wordt de route rustiger en worden pijnlijke of onnodige vragen vermeden.</p></div></div><NextStep title={next.title} text={next.text} primary={next.primary} onPrimary={goNext}/></section>}
