...
const flowSteps=[
 ...
 {key:"home",label:"Mijn woning",page:"home"},
...
function getStepIndex(page,wizard){
 if(page==="profile")return Math.max(0,Math.min(5,wizard));
 const idx=flowSteps.findIndex(s=>s.page===page);
 return idx>=0?idx:0;
}
function stepTitle(page,wizard){
 return flowSteps[getStepIndex(page,wizard)]?.label||"ATLAS Kooproute";
}
function JourneyFlow({page,wizard,setPage,setWizard,s}){
 const idx=getStepIndex(page,wizard);
 const progress=Math.round(((idx+1)/flowSteps.length)*100);
 return <div className="journeyFlow"><div className="journeyHead"><div><span>ATLAS Kooproute</span><b>Stap {idx+1} van {flowSteps.length} — {flowSteps[idx].label}</b></div><em>{progress}% naar biedklaar</em></div><div className="flowBar"><i style={{width:progress+"%"}}/></div><div className="flowDots">{flowSteps.map((st,i)=><button key={st.key} onClick={()=>{setPage(st.page); if(st.page==="profile")setWizard(st.wizard)}} className={i<idx?"done":i===idx?"active":""}>{i<idx?"✓":i+1}<span>{st.label}</span></button>)}</div></div>
}
...
function getGlobalNext(page,wizard,s,buyer){
 if(page==="profile"&&wizard<5)return {title:`Stap compleet`,text:`Je bent bij stap ${wizard+1} van 12. Ga door naar ${flowSteps[wizard+1].label}.`,primary:`Ga naar ${flowSteps[wizard+1].label}`,page:"profile",wizard:wizard+1};
 if(page==="profile"&&wizard===5)return s.docs<80?{title:"Documentstatus is nog laag",text:"Rond eerst je Document Vault af voordat ATLAS je kooppositie betrouwbaar kan maken.",primary:"Ga naar Documenten",page:"documents"}:{title:"Intake compleet",text:"Je gegevens staan klaar. Controleer nu je huidige woning of ga door naar marktinzicht.",primary:"Check mijn woning",page:"home",secondary:"Bekijk Marktinzicht",secondaryPage:"market"};
...
function App(){
 ...
 const journey=[["Persoonlijk",s.personal,"profile"],["Gezin",s.family,"profile"],["Werk",buyer.income?96:35,"profile"],["Eigen woning",s.home,"home"],["Financiën",s.financial,"profile"],["Documenten",s.docs,"documents"],["Adviesprofiel",s.advice,"advice"],["Marktinzicht",82,"market"],["Woningen",91,"matches"],["Paspoort",68,"share"]];
 ...
 return <div className="app"><aside><div className="brand"><div className="mark">A</div><div><b>ATLAS</b><span>NEXT 2.6</span></div></div><button className="newUser" onClick={()=>setProfileMenu(!profileMenu)}><Plus/> Nieuwe koper</button>{profileMenu&&<div className="newBox"><b>Start nieuw profiel</b>{Object.entries(profiles).map(([id,p])=><button key={id} onClick={()=>start(id)}>{p.label}</button>)}</div>}<div className="journey">{journey.map(([name,sc,target],i)=><button key={name} onClick={()=>{setPage(target); if(target==="profile")setWizard(Math.min(i,5))}} className={page===target?"routebtn active":"routebtn"}><span className={sc>=80?"done":sc>=60?"busy":"todo"}>{sc>=80?"✓":i+1}</span><div><b>{name}</b><small>{sc}%</small></div></button>)}</div><nav>{[["cockpit",Compass,"Cockpit"],["profile",UserRound,"Gegevens"],["home",HousePlus,"Mijn woning"],["documents",UploadCloud,"Documenten"],["advice",ShieldAlert,"Adviesprofiel"],["market",BarChart3,"Marktinzicht"],["matches",Home,"Matches"],["coach",Bot,"Koopcoach"],["share",Share2,"Delen"]].map(([id,Icon,label])=><button key={id} onClick={()=>setPage(id)} className={page===id?"on":""}><Icon/> {label}</button>)}</nav></aside><main><JourneyFlow page={page} wizard={wizard} setPage={setPage} setWizard={setWizard} s={s}/>{page==="cockpit"&&<Cockpit s={s} setPage={setPage} selected={selected} buyer={buyer}/>} {page==="profile"&&<Profile buyer={buyer} update={update} s={s} wizard={wizard} setWizard={setWizard} next={next} goNext={goNext} goSecondary={goSecondary}/>} {page==="home"&&<MyHome buyer={buyer} updateBuyer={updateBuyer} s={s} docs={docsForBuyer} setDocs={setDocs} next={next} goNext={goNext}/>} {page==="documents"&&<Documents docs={docsForBuyer} setDocs={setDocs} s={s} buyer={buyer} next={next} goNext={goNext}/>} {page==="advice"&&<Advice buyer={buyer} update={update} s={s} next={next} goNext={goNext}/>} {page==="market"&&<Market buyer={buyer} s={s} setPage={setPage} next={next} goNext={goNext}/>} {page==="matches"&&<Matches homes={listings.map(h=>adjustListing(h,s,buyer))} homeId={homeId} setHomeId={setHomeId} selected={selected} s={s} next={next} goNext={goNext}/>} {page==="coach"&&<Coach buyer={buyer} s={s} selected={selected} setPage={setPage} next={next} goNext={goNext}/>} {page==="share"&&<Share buyer={buyer} s={s} selected={selected} next={next} goNext={goNext}/>}</main></div>
}
...
function Profile({buyer,update,s,wizard,setWizard,next,goNext,goSecondary}){
 ...
 {wizard===5&&<><h3>Resultaat intake</h3><div className="resultCards"><Metric label="ATLAS Score" value={s.atlas}/><Metric label="Koopruimte" value={short(s.budget)}/><Metric label="Overwaarde" value={euro(s.equity)}/><Metric label="Documentstatus" value={s.docs+"%"}/></div><p>Volgende stap: rond de dynamische documenten af en bekijk je marktinzicht.</p></>}
 <div className="wizardActions"><button className="secondary" disabled={wizard===0} onClick={()=>setWizard(Math.max(0,wizard-1))}>Terug</button>{wizard<5&&<button onClick={()=>setWizard(Math.min(tabs.length-1,wizard+1))}>Ga naar {tabs[Math.min(tabs.length-1,wizard+1)]}<ChevronRight size={16}/></button>}</div><NextStep title={next.title} text={next.text} primary={next.primary} secondary={next.secondary} onPrimary={goNext} onSecondary={goSecondary}/></div><LivePanel s={s}/></div></section>
}
...
function LivePanel({s}){return <aside className="livePanel"><h3>Live effect</h3><strong>{s.atlas}</strong><Badge value={s.atlas}>{s.atlas>=82?"Koopklaar":"Bijna koopklaar"}</Badge><Bar value={s.atlas}/><Metric label="Indicatieve bandbreedte" value={short(s.budget)}/><Metric label="Confidence" value={s.confidence+"%"}/><Metric label="Betrouwbaarheid woningwaarde" value={s.homeReliability+"%"}/><div className="tip"><Bot size={18}/> Nog nodig: documenten, BKR-check en marktfit.</div></aside>}
